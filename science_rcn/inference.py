"""
A reference implementation of max-product loopy belief propagation inference for a two-level RCN model.

This code is an unoptimized version of what was used to produce the results in the paper.
Note that we use a faster implementation of 2D dilation, instead of the slower scipy.ndimage.morphology.grey_dilation.
"""
import numpy as np
...
def test_image(img, model_factors, pool_shape=(25, 25), num_candidates=20, n_iters=300, damping=1.0):
    """
    ...
    for idx in top_candidates:
        ...
        rcn_inf = LoopyBPInference(bu_msg, frcs, edge_factors, pool_shape, preproc_layer, n_iters, damping=damping)
...
class LoopyBPInference(object):
    ...
    def __init__(self, bu_msg, frcs, edge_factors, pool_shape, preproc_layer, n_iters=300, damping=1.0, tol=1e-5):
        ...
        if (frcs.max(0)>=np.array([self.n_feats, self.n_rows-((self.vps-1)//2), self.n_cols-((self.hps-1)//2)])).any():
            raise RCNInferenceError("Some frcs are too big for the provided pool shape and/or `bu_msg`")
